...
find -H ${DIR} -maxdepth 1 -type f -name '[A-Za-z0-9]*' | \
    xargs echo ${DIR}/*/*.hin ${DIR}/*/*.[hc] ${DIR}/config/*.py ${DIR}/tools/*.py ${DIR}/cmake/*.cmake ${DIR}/*/*.f90 ${DIR}/*/*.lua ${DIR}/*/doxygen* ${DIR}/share/pkgconfig/plasma.pcin ${DIR}/share/cmake/plasma.cmakein |  \
    xargs tar --exclude=.hgtags --exclude=plasma_config.h --exclude=Makefile.\*.gen --owner=root --group=root --mtime=1970-01-01 -chof ${DIR}.tar
