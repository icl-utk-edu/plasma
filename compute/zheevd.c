    ...
    plasma_complex64_t *pQ, int ldq)
{
    ...
    plasma_omp_zheevd(
        job, uplo, A, *T, Lambda, pQ, ldq, work,
        &sequence, &request);
    ...
    plasma_sequence_t *sequence, plasma_request_t *request)
{
    ...
    #pragma omp master
    {
        ...
        plasma_pzhecpy_tile2lapack_band(
            uplo,
            A,
            A_band, lda_band,
            sequence, request);
    ...
    if (job == PlasmaVec) {
        ...
        if (nb < n) {
            plasma_desc_t Q;
            plasma_desc_general_create(
                PlasmaComplexDouble, nb, nb,
                n, n, 0, 0, n, n, &Q);
